...
LRU 替换策略已经在 Cache Lab 中实现过，因此本部分侧重介绍如何在确保线程安全的前提下实现对缓存的读取逐出操作，主要使用了引用计数的 Arc 指针配合 Mutex 实现
...
对于 Proxy 程序，可以看到第二次之后的相同请求通过 Cache 内容应答（显示 by cache）：
...
实际网页浏览效果（这里选择浏览学校提供的个人主页服务，采用的是 http 协议）
...
web proxy 缓存是很经典的读者-写者问题，但当指定使用 LRU 机制后，问题会变得复杂一点：LRU 的读取操作实际也会更新 LRU 的数据结构，所以读取和写入此时仅仅靠读写锁已经无法保证安全了，而需要使用更一般的互斥锁，读取写入操作都对 LRU 加锁

LRU 存放的是键值对，键显然取 URI 用以哈希，而值的类型这里选择了 `Arc<Vec<u8>>`，Arc 相当于引用计数的智能指针，这是为了处理某条消息同时在发送和要被逐出的情况，这时，只有当消息发送完成，对应消息的引用计数归 0 后，资源才会被回收，避免了悬空指针的出现

注：如果这里不用 Arc 包裹甚至是无法通过 Rust 的语法检查的，从中也可以看出 Rust 对内存安全要求的严格
...
这里 LRU 缓存设置为最大保存 32 个消息（对象）

cache 为了能在协程间共享，最外层也要用 Arc 包裹，因此每次需要将该智能指针复制一份后传入新的协程：
...
协程在接收到客户端的 http 请求并解析后会查询缓存中是否存在所需要的值：
...
这里注意 `.cloned()` 是将返回消息的智能指针复制了一份，对 LRU 的锁在获取到这个智能指针后即可释放

如果缓存中有相关数据，那么直接发送给客户端后即可关闭连接，否则再执行与服务端建立连接等操作
...
这里使用了 buffer_valid 来限制 buffer 大小，当超过 1MiB 后，就不再存入 buffer，也不会放入缓存

...
理论上来说这里需要阻塞，实际上，多核 cpu 并行运行进程，在进程数小的情况下很难因为并发遇到执行时序的问题，不过严谨考虑还是加锁为好，这里作为一个 toy 程序就没加了
...
这实际上是因为如果 shell 中运行的程序如果因为自身收到终止 / 暂停信号而终止，我们希望 shell 程序也能提示用户。而信号处理函数只能处理 shell 程序收到的信号，例如键盘输入，适用范围就窄了

如果把打印写在信号处理函数内，功能依旧正常，但是会在 `test16` 中因为没有子进程终止信号的提示，输出与 `tshref` 不一致
